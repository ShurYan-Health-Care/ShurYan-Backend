...
namespace Shuryan.Application.Services
{
    public class PharmacyService : IPharmacyService
    {
        ...
        public async Task<PharmacyWorkingHoursResponse> AddWorkingHoursAsync(Guid pharmacyId, CreatePharmacyWorkingHoursRequest request)
        {
            ...
            var hours = new PharmacyWorkingHours { Id = Guid.NewGuid(), PharmacyId = pharmacyId, DayOfWeek = request.DayOfWeek, StartTime = TimeOnly.FromTimeSpan(request.StartTime), EndTime = TimeOnly.FromTimeSpan(request.EndTime), CreatedAt = DateTime.UtcNow };
        ...
        public async Task<IsOpenResponse> IsPharmacyOpenAsync(Guid pharmacyId)
        {
            ...
            return new IsOpenResponse { PharmacyId = pharmacyId, IsOpen = isOpen, Message = isOpen ? "الصيدلية مفتوحة حالياً" : "الصيدلية مغلقة حالياً", CurrentTime = currentDateTime, OpeningTime = todayHours?.StartTime.ToTimeSpan(), ClosingTime = todayHours?.EndTime.ToTimeSpan() };
        }

        public async Task<PharmacyOrderResponse> AcceptOrderAsync(Guid pharmacyId, Guid orderId) => await UpdateOrderStatusInternalAsync(pharmacyId, orderId, PharmacyOrderStatus.Confirmed);
        ...
        public async Task<PharmacyOrderResponse> MarkOrderAsPreparingAsync(Guid pharmacyId, Guid orderId) => await UpdateOrderStatusInternalAsync(pharmacyId, orderId, PharmacyOrderStatus.PreparationInProgress);
        public async Task<PharmacyOrderResponse> MarkOrderAsReadyAsync(Guid pharmacyId, Guid orderId) => await UpdateOrderStatusInternalAsync(pharmacyId, orderId, PharmacyOrderStatus.ReadyForPickup);
        public async Task<PharmacyOrderResponse> DispatchOrderAsync(Guid pharmacyId, Guid orderId) => await UpdateOrderStatusInternalAsync(pharmacyId, orderId, PharmacyOrderStatus.OutForDelivery);
        ...
        public async Task<IEnumerable<PharmacyOrderResponse>> GetInProgressOrdersAsync(Guid pharmacyId)
        {
            ...
            var inProgress = allOrders.Where(o => o.Status == PharmacyOrderStatus.Confirmed || o.Status == PharmacyOrderStatus.PreparationInProgress || o.Status == PharmacyOrderStatus.OutForDelivery || o.Status == PharmacyOrderStatus.ReadyForPickup);
        ...
        public async Task<IEnumerable<PharmacyResponse>> SearchPharmaciesAsync(SearchPharmaciesRequest request)
        {
            ...
            foreach (var pharmacy in list) { var reviews = await _unitOfWork.PharmacyReviews.GetByPharmacyIdAsync(pharmacy.Id); var reviewsList = reviews.ToList(); pharmacy.AverageRating = reviewsList.Any() ? reviewsList.Average(r => r.AverageRating) : null; pharmacy.TotalReviewsCount = reviewsList.Count; }
            if (request.MinRating.HasValue) list = list.Where(p => p.AverageRating.HasValue && p.AverageRating.Value >= request.MinRating.Value).ToList();
        ...
        public async Task<IEnumerable<PharmacyResponse>> GetNearbyPharmaciesAsync(NearbyPharmaciesRequest request)
        {
            ...
            var pharmaciesWithAddress = allPharmacies.Where(p => p.Address != null && p.Address.Latitude.HasValue && p.Address.Longitude.HasValue && p.VerificationStatus == VerificationStatus.Verified).ToList();
            var nearby = new List<(Core.Entities.Identity.Pharmacy Pharmacy, double Distance)>();
            foreach (var pharmacy in pharmaciesWithAddress) { var distance = CalculateDistance(request.Latitude, request.Longitude, pharmacy.Address!.Latitude!.Value, pharmacy.Address.Longitude!.Value); if (distance <= request.RadiusInKm) nearby.Add((pharmacy, distance)); }
        ...
        {
            const double R = 6371; var dLat = ToRadians(lat2 - lat1); var dLon = ToRadians(lon2 - lon1);
        ...
        {
            var reviews = await _unitOfWork.PharmacyReviews.GetByPharmacyIdAsync(pharmacyId); var list = reviews.ToList();
            return new PharmacyReviewStatisticsResponse { PharmacyId = pharmacyId, TotalReviewsCount = list.Count, AverageRating = list.Any() ? list.Average(r => r.AverageRating) : 0, FiveStarCount = list.Count(r => r.AverageRating >= 4.5), FourStarCount = list.Count(r => r.AverageRating >= 3.5 && r.AverageRating < 4.5), ThreeStarCount = list.Count(r => r.AverageRating >= 2.5 && r.AverageRating < 3.5), TwoStarCount = list.Count(r => r.AverageRating >= 1.5 && r.AverageRating < 2.5), OneStarCount = list.Count(r => r.AverageRating < 1.5), AverageOverallSatisfaction = list.Any() ? list.Average(r => r.OverallSatisfaction) : 0, AverageMedicationAvailability = list.Any() ? list.Average(r => r.MedicationAvailability) : 0, AverageServiceQuality = list.Any() ? list.Average(r => r.ServiceQuality) : 0, AverageDeliverySpeed = list.Any() ? list.Average(r => r.DeliverySpeed) : 0, AverageValueForMoney = list.Any() ? list.Average(r => r.ValueForMoney) : 0 };
        }
        ...
        public async Task<PharmacyResponse> VerifyPharmacyAsync(Guid pharmacyId, VerifyPharmacyRequest request)
        {
            ...
            pharmacy.VerificationStatus = VerificationStatus.Verified; pharmacy.VerifiedAt = DateTime.UtcNow; pharmacy.VerifierId = request.VerifierId; pharmacy.UpdatedAt = DateTime.UtcNow;
            _unitOfWork.Pharmacies.Update(pharmacy); await _unitOfWork.SaveChangesAsync();
        ...
        {
            var pharmacy = await _unitOfWork.Pharmacies.GetByIdAsync(pharmacyId) ?? throw new KeyNotFoundException();
            pharmacy.VerificationStatus = VerificationStatus.Rejected; pharmacy.UpdatedAt = DateTime.UtcNow;
            _unitOfWork.Pharmacies.Update(pharmacy); await _unitOfWork.SaveChangesAsync();
            return await GetPharmacyByIdAsync(pharmacyId) ?? throw new Exception();
        }
        ...
        public async Task<PharmacyDocumentResponse> ApproveDocumentAsync(Guid pharmacyId, Guid documentId, ApproveDocumentRequest request)
        {
            ...
            doc.Status = VerificationDocumentStatus.Approved; doc.UpdatedAt = DateTime.UtcNow;
            _unitOfWork.PharmacyDocuments.Update(doc); await _unitOfWork.SaveChangesAsync();
            return _mapper.Map<PharmacyDocumentResponse>(doc);
        }
        ...
        public async Task<PharmacyDocumentResponse> RejectDocumentAsync(Guid pharmacyId, Guid documentId, RejectDocumentRequest request)
        {
            var doc = await _unitOfWork.PharmacyDocuments.GetByIdAsync(documentId) ?? throw new KeyNotFoundException();
            if (doc.PharmacyId != pharmacyId) throw new UnauthorizedAccessException();
            doc.Status = VerificationDocumentStatus.Rejected; doc.RejectionReason = request.RejectionReason; doc.UpdatedAt = DateTime.UtcNow;
            _unitOfWork.PharmacyDocuments.Update(doc); await _unitOfWork.SaveChangesAsync();
            return _mapper.Map<PharmacyDocumentResponse>(doc);
        }
    }
